import { dev } from '$app/environment';
export const BASE_URL = dev ? 'http://localhost:5173' : 'https://sveltekit-auth-alpha.vercel.app';
export const APP_NAME = 'Thumbs';
export const CONTACT_EMAIL = 'contact@email.com';
export const DOMAIN = 'sveltekit-auth-alpha.vercel.app';

/* WARNING!!! TERMS AND CONDITIONS AND PRIVACY POLICY 
WERE CREATED BY CHATGPT AS AN EXAMPLE ONLY. 
CONSULT A LAWYER AND DEVELOP YOUR OWN TERMS AND PRIVACY POLICY!!! */
export const TERMS_PRIVACY_CONTACT_EMAIL = 'contact@email.com';
export const TERMS_PRIVACY_WEBSITE = 'somedomain.com';
export const TERMS_PRIVACY_COMPANY = 'Thumbs';
export const TERMS_PRIVACY_EFFECTIVE_DATE = 'January 1, 2024';
export const TERMS_PRIVACY_APP_NAME = 'Thumbs';
export const TERMS_PRIVACY_APP_PRICING_AND_SUBSCRIPTIONS =
	'[Details about the pricing, subscription model, refund policy]';
export const TERMS_PRIVACY_COUNTRY = 'Finland';
